...
FRASbiz is a face recognition-based attendance management system. Using MTCNN for face detection and face_recognition library for face matching, this system automatically identifies individuals and logs attendance for each session. This project can be used for live capturing or from an uploaded photo, aiming to simplify attendance taking in classrooms, offices, or other group environments.
...
Virtual Environment (Recommended): It's advised to use a virtual environment to manage dependencies.
Steps
...
Live Capture Mode
Navigate to the Live Attendance Capture section.
Click Capture to take a photo from the webcam.
The system will automatically detect and identify faces, display bounding boxes, and log attendance.
Attendance data is saved to the specified CSV file.
Upload Photo Mode
Upload a group photo for attendance logging.
The system will process the image, identify known faces, and log attendance accordingly.
Attendance data is updated in real-time and displayed in the interface.
Configuration Files
